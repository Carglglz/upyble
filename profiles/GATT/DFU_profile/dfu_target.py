...
_DEVICE_FIRMWARE_UPDATE_SERV_UUID = bluetooth.UUID("00001530-1212-efde-1523-785feabcd123")
        ...
        if event == _IRQ_CENTRAL_CONNECT:
            ...
            if conn_handle in self._connections and value_handle == self._dfu_control_point:
                ...
                if opcode == self._opcodes_dict['START_DFU']:
                    self._received_image_len = 0
                    self._opcode, self._image_type = struct.unpack('BB', self._ble.gatts_read(self._dfu_control_point))
                    print('START_DFU (0x01) OPCODE RECEIVED')
                    print('IMAGE TYPE: {}'.format(self._dfu_image_type_codes[self._image_type]))
                elif opcode == self._opcodes_dict['Initialize DFU Parameters']:
                    self._opcode, init_code = struct.unpack('BB', self._ble.gatts_read(self._dfu_control_point))
                    ...
                    self._opcode, = struct.unpack('B', self._ble.gatts_read(self._dfu_control_point))
                    ...
                    with open(self._filename, 'rb') as filetocheck:
                        data = filetocheck.read()
                        checksum = crc16xmodem(data)
                    print('Indicated Checksum: {} | Received Checksum: {}'.format(self._image_checksum, checksum))
            ...
            # DFU PACKET
                ...
                if self._opcode == self._opcodes_dict['START_DFU']:
                    len_sofd, len_bootl, len_app = struct.unpack('III', self._ble.gatts_read(self._dfu_packet))
                    print('IMAGE SIZE (bytes): SOFTDEVICE: {}, BOOTLOADER: {}, APPLICATION: {}'.format(len_sofd, len_bootl, len_app))
                    ...
                    n_soflen = struct.unpack('HHIH', init_packet[:struct.calcsize('HHIH')])[-1]
                    ...
                    print('Device type | Device Rev | App Version | Softdevice len | Softdevice | Checksum (CRC-16-CCITT)')
                    print('{} | {} | {} | {} | {} | {}'.format(*self._init_packet))  # FIXME: ++softdevices
                    ...
                    if self._name_file_packet:
                        ...
                        with open(self._filename, 'ab') as fwfile:
                            fwfile.write(data_packet)
