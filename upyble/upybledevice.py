...
class DFU_BLE_CONTROLLER(BASE_BLE_DEVICE):
    ...
    def load_image(self, img, len_application=0, len_softdevice=0, len_bootloader=0):
        with open(img, 'rb') as imagefile:
            ...
            n_soflen = struct.unpack('HHIH', self._image_raw_data[:struct.calcsize('HHIH')])[-1]
            self._init_packet_len = struct.calcsize('HHIH') + n_soflen * struct.calcsize('H') + struct.calcsize('H')
    ...
    def get_image_size(self):
        self._image_size = self._soft_device_len + self._bootloader_len + struct.pack(self._size_format, self._application_len)
    ...
    def write_init_packet(self):
        ...
        init_packet_code = struct.pack('B', self._init_packet_codes['Receive Init Packet'])
        ...
        init_complete_packet_code = struct.pack('B', self._init_packet_codes['Init Packet Complete'])
        response_opcode_init_complete = self.write_read_opcode(byte_opcode+init_complete_packet_code, 3)
...
def dfufy_file(file):
    ...
    try:
        with open(file, 'rb') as filetofreeze:
            ...
            # data_to_check = file.encode('utf8') + b'\n' + data
            checksum = crc16xmodem(data)
        init_packet = get_init_packet([1, 1, 1, 1, 1, checksum])
        with open(file.split('.')[0] + '.bin', 'wb') as filetobin:
            namefile_data = file.encode('utf8') + b'\n' + data
            image_data = init_packet + namefile_data
            filetobin.write(image_data)
        print('File {} ready!'.format(file.split('.')[0] + '.bin'))
    except Exception as e:
        print(e)
